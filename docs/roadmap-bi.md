...
| V | **`cliente_status_diario` do Pulse guarda só 4 dias — e ninguém arquiva.** Tabela de status diário por empresa (10.419 linhas, 2.616 empresas), com histórico de 10/08 a 13/08 apenas. É um retrato diário de cliente ativo/plano que **se perde todo dia**, exatamente como a navegação da plataforma antes de nós. Se espelharmos, o BI passa a ser a única série histórica que existe; se não, ela continua sumindo | CS | pedir ao Pulse a inclusão no contrato `bi_pulse` — é o pedido de maior retorno em aberto |
