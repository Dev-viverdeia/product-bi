...
- **Os dez módulos continuam sendo a navegação.** As camadas viram **aba** dentro de cada um: `Análise` · `Gráficos` · `Dados`. Fazer as três de topo quebraria o link "ver o gráfico que sustenta", que é o que amarra a frase ao número.
- **`Plano de ação` (`/plano`) e `Explorar` (`/explorar`) são seções de topo**, porque são as únicas transversais. As duas ficam no grupo `panorama` do rail.
- **`Plano de ação` agrega, não recalcula.** `bi_plano_de_acao` junta os achados JÁ calculados de cada tela e ordena por score. A cegueira entre telas do motor **continua valendo no cálculo** — o contrato de CI não foi afrouxado. A lista de telas sai de `insights.regra` e o nome da RPC é derivado do slug: tela nova com regra entra sozinha, e se a RPC faltar a função **aborta** em vez de omitir em silêncio.
  ...
  - ⚠️ **O motor está saturado** (34 de 35 regras dispararam), então a **ordem** vale mais que a presença. A tela declara isso; recalibrar limiar é decisão em aberto.
- **Aba `Dados`: nenhuma consulta nova.** `AbaDeDados` recebe as linhas JÁ carregadas pelos mesmos hooks que desenham os gráficos. Se ela relesse o banco, passaria a existir uma segunda consulta capaz de divergir do card ao lado — o defeito que o projeto vem fechando em outras camadas. A garantia é estrutural: é o mesmo objeto em memória. **Estado: só na Visão Geral**, como peça de referência.
