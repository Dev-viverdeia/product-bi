...
import { ListaDeAcao } from "@/components/tabela/lista-de-acao";
import type { ItemDeAcao } from "@/components/tabela/lista-de-acao";
...
/*
  Os quatro degraus, na ordem em que aparecem na tela. Vivem aqui e não numa
  constante compartilhada de propósito: o showcase existe para EXERCITAR o
  token, então ele referencia a utility (`bg-moldura`) em vez do valor. Se um
  degrau sumir do tema, o quadrado fica transparente e o defeito aparece.
*/
const SUPERFICIES = [
  {
    nome: "página",
    classe: "bg-background",
    papel: "margem em volta da moldura",
  },
  {
    nome: "moldura",
    classe: "bg-moldura",
    papel: "o quadro — e a cor da aba ativa",
  },
  {
    nome: "seção",
    classe: "bg-secao",
    papel: "agrupa cards da mesma pergunta",
  },
  {
    nome: "cromo · card",
    classe: "bg-card",
    papel: "barra, rail e card — o único branco",
  },
] as const;
...
const clientesEmRisco: ItemDeAcao[] = [
  {
    id: "1",
    titulo: "Ricardo Souza",
    subtitulo: "Órbita Digital · Pro",
    valor: "47 d",
  },
  {
    id: "2",
    titulo: "Carla Menezes",
    subtitulo: "Vetor Consultoria · Enterprise",
    valor: "39 d",
  },
  {
    id: "3",
    titulo: "André Pacheco",
    subtitulo: "Casa Nova · Pro",
    valor: "31 d",
  },
];
...
export function DesignPage() {
  ...
  const [recorteDemo, setRecorteDemo] =
    useState<(typeof RECORTES_DEMO)[number]["valor"]>("todos");
  ...
  const melhorMes = receitaMensal.reduce((a, b) =>
    Number(b.receita) > Number(a.receita) ? b : a,
  );
  const canalLider = sessoesPorCanal.reduce((a, b) =>
    b.value > a.value ? b : a,
  );
  const totalSessoes = sessoesPorCanal.reduce((soma, c) => soma + c.value, 0);
  const produtoLider = receitaPorProduto.reduce((a, b) =>
    b.value > a.value ? b : a,
  );
  const totalAssinaturas = assinaturasPorPlano.reduce(
    (soma, p) => soma + p.value,
    0,
  );
  ...
  const picoHeatmap = dadosHeatmap.reduce(
    (maior, c) => Math.max(maior, c.valor),
    0,
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Showcase do kit de gráficos — dados de exemplo · o recorte ao lado é
            o componente real e escreve na URL
          </p>
        </div>
        <SegmentoFiltro />
      </div>
      ...
      {/* A seção é o degrau de hierarquia entre a tela e o card: ela agrupa os
          blocos que respondem à MESMA pergunta. Aqui ela também serve de prova
          da rampa — o cinza da seção precisa se distinguir da moldura atrás e
          dos cards brancos dentro, nos dois temas. */}
      <SecaoDeAnalise
        titulo="Fundação do layout"
        icone={LayersIcon}
        descricao="Peças do shell fora do kit de gráficos. A seção é o contêiner desta caixa cinza; o segmentado e a lista de ação são os componentes dentro dela."
        controles={
          <ControleSegmentado
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <ChartCard
          ...
          headline={
            ultimoMes
              ? formatCompact(
                  Number(ultimoMes.recorrentes) + Number(ultimoMes.novos),
                )
              : "—"
          }
