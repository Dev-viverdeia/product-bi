...
- ⚠️ **Não rodar `npx prettier --write`.** O projeto não tem `.prettierrc`; o estilo (aspas simples, sem ponto e vírgula) vive só no ESLint. Sem config, o prettier reescreve o arquivo inteiro nos padrões dele e o diff some no ruído.
...
- Todo controle tem os 3 estados (`:hover`, `:active`, `:focus-visible`); disabled muta por cor, não por opacity. Contraste AA nos 2 temas, sempre.
  - ⚠️ **`--ring` é SÓLIDO, e o valor saiu de medição.** A receita `rgba(10,31,59,0.35)` compunha 2,17:1 sobre o card e 2,11:1 sobre a seção — reprovando o piso de 3:1 nas quatro superfícies do claro. Pior: `focus-visible:ring-ring/50` no `button.tsx` dividia de novo (1,42:1 no claro, 2,07:1 no escuro, que sozinho passava). Hoje `#1e3a5f`, medido em 11,50 · 9,91 · 9,22 · 9,57. **Anel com alfa tem contraste que depende da superfície de baixo — não usar.**
...
- `CardCabecalho` (`src/components/ui-marca/`) — **uma gramática de cabeçalho para todo card de conteúdo**: identidade à esquerda, afordância à direita. `icon` + título curto numa linha, `headline` com o número que responde o card (mais `headlineLabel`), e a `description` vira o conteúdo do botão circular de informação — a definição da métrica não sai da tela, sai do caminho. `action` substitui o botão quando o card tem controle próprio (o seletor de tela na Jornada).
  - ⚠️ **É `Popover`, não `Tooltip`** — tooltip do Radix não abre no toque, e eram 79 réguas de card inertes no celular. E o primitivo estava errado por um segundo motivo: tooltip é para rótulo curto, some ao perder foco e não dá para selecionar; ali o conteúdo é a régua da métrica, texto de quatro linhas que alguém lê com calma. Importado direto de `radix-ui` em `ui-marca/`, como o `AcordeaoDeAchados` — `src/components/ui/` é território do gerador.
  - ⚠️ **`description` é DESCARTADA em silêncio quando `action` também é passada.** As duas ocupam o mesmo slot à direita. Card com controle próprio perde a régua sem aviso nenhum.
...
- `KpiCard`/`KpiGrid` — stat tile com count-up, delta assinado (`upIsGood` decide a cor), sparkline; grid de **duas colunas já no menor tamanho** (com o `sm:` os quatro tiles empilhavam em ~512px e a primeira dobra do celular era inteira de cromo, sem um número). Três estados de exceção, todos distintos: erro ("não foi possível carregar"), e **sem amostra** (`value: null` + `motivoSemValor` — travessão com o porquê, para percentual suprimido pela régua de 30). Nunca `?? 0` em valor de KPI.
...
- `CategoryBarChart` — colunas ou barras horizontais; ≤24px, ponta 4px, base no zero, valor na ponta; eixos `width="auto"` (largura fixa corta rótulo).
  - ⚠️ **`value` é `number | null`, e `?? 0` na página é proibido** (travado no CI). Zero é uma medida, ausência não é — é a mesma doutrina do `KpiCard`, que o gráfico não tinha. Suprimido = barra ausente, categoria no eixo, `motivoSemValor` declarado **em texto abaixo do gráfico**: o canal é HTML e não geometria do Recharts, porque barra nula não tem retângulo onde pendurar rótulo nem área onde disparar tooltip. Custou um card de destaque em Formações que desenhava "0,0%" e se contradizia com a própria descrição. Antes de escrever o motivo, **ler a cláusula de supressão da RPC no banco** — `count(*) >= 10`, `total >= 30`, `having >= 100` — porque duas das cinco coerções eram sobre funções que nunca devolvem nulo. `nota` por dado leva uma **segunda medida só para o tooltip** — é anotação, nunca segunda série nem segundo eixo, e chega já formatada com a unidade escrita. Existe para o caso em que a barra responde "quanto" e a análise escrita afirma "que proporção": sem esse canal o texto publica um número que o gráfico ancorado não mostra, e quem clicou para conferir não encontra o que veio conferir.
...
**Carregamento: o dado anterior fica, e ele DIZ que é anterior.** `placeholderData: keepPreviousData` e `staleTime` de 5 min são padrão global do QueryClient (`app/providers.tsx`) — antes, trocar 30→90 dias derrubava os doze cards de Clientes para esqueleto ao mesmo tempo, e como o esqueleto é menor que o conteúdo a página saltava. "Poluído" tem um componente que não é tinta.

- ⚠️ **`isRefreshing` é o par OBRIGATÓRIO disso**, no `ChartCard` e no `TabelaCard`: `q.isFetching && !!q.data`. Sem ele o número da janela anterior fica na tela com cara de número da janela atual — o keepPreviousData vira defeito novo. Card novo nasce com os dois ou com nenhum.
- **Duas exceções, declaradas no próprio hook.** `useAchados` (o resultado é PROSA QUE AFIRMA NÚMEROS: segurar o anterior deixaria o parágrafo da janela velha publicado ao lado de KPIs já atualizados) e `usePaginaBruta` (a chave carrega tabela E offset — o global manteria as linhas da tabela anterior sob um cabeçalho novo, que não parece carregamento, parece resultado).
- 5 min e não 60 s porque o pipeline escreve a cada 30 min: refetch de minuto em minuto trazia os mesmos bytes. É o passo que `FrescorDoDado` e `AlertaPipeline` já usavam.
...
- **Zero dígito no gabarito.** A régua viaja em `parametros` emitido pela mesma função que calcula — mude a janela no SQL e a frase acompanha. Teste no CI reprova.
  - ⚠️ **O teste tinha um cego até 20/ago: ele só via literal que CARREGA MARCADOR.** Frase sem marcador nenhum era invisível — e é justo onde o número escrito na mão tem mais chance de estar, porque quem não usou marcador é porque escreveu o valor. Pior: **correção de texto vira `update`, não `insert`**, então a metade do catálogo que mais muda escapava inteira. Passou um `1` de "Usar IA na 1ª semana" citado como nome de card, com o CI verde. Fechado com uma segunda extração por nome de coluna (`gabarito_acao = '…'`) — 170 → 214 testes, e nenhuma violação histórica apareceu.
  - Consequência prática: **card com dígito no nome não se cita pelo título dentro de um gabarito.** Usar referência descritiva, como já fazia `vg_tendencia` ("o card de origem dos ativos, nesta tela").
...
**A navegação voltou para o lado, e a reversão tem motivo medido.** O CLAUDE.md dizia "navegação no topo, a sidebar foi removida — não reintroduzir". O que estava errado era a *sidebar*, não o *lado*: ela era ícone **+ rótulo**, e a barra horizontal que a substituiu somava 1.189px de rótulo em dez módulos, só cabendo a partir de `xl`. O rail passou a ser vertical, e as abas do módulo saíram da página para dentro da barra — que é o que devolve a banda vertical que a sidebar antiga custava.

- ⚠️ **O RÓTULO VOLTOU AO RAIL EM 19/AGO, POR DECISÃO DO CEO, e isto substitui a linha "não reintroduzir sidebar com rótulo".** O que sustentava a versão só de ícone era o tooltip, e o próprio comentário do componente admitia o problema: "sem ele o rail vira adivinhação". Menu que só se lê passando o mouse cobra uma interação para responder "onde estou e para onde posso ir", e quem abre o BI uma vez por semana paga esse pedágio toda vez.
  - **O custo foi medido, não estimado, e é o argumento que perdeu**: o rail foi de 68px para **208px** (`w-52`), e a largura sai do conteúdo — a 1720px do shell os gráficos caem de 1612px para **1472px (−140px, −8,7%)**. Contraria a mesma prioridade que encolheu a calha de 20px para 12px. A decisão do CEO tem precedência; o registro existe para a troca ser conhecida, não para ser refeita por conta própria.
  - **A largura sai da MEDIÇÃO do rótulo mais longo, não de estimativa.** Os 14 rótulos foram medidos na fonte real (Outfit variável do `@fontsource-variable`, 14px, com o tracking `-0.011em` do base): o pior é `Consultor & Builder` com **~118px numa caixa de 140px** — 84% de ocupação, 22px de folga. A estimativa a olho dizia 139px e teria custado 24px de gráfico à toa. **Dimensionar coluna de texto por chute é como decidir cor no olho: mede-se.** O `truncate` no rótulo é rede, não plano — módulo novo com nome maior corta com reticências em vez de empurrar o layout.
  - Abaixo de `lg` nada muda: o rail continua virando gaveta (`Sheet`), que **já era rotulada** desde sempre.
- ⚠️ **O separador de grupos nunca chegou a ser desenhado, de 13 a 19/ago.** Era `<span className="h-3 first:hidden" />` como **primeiro filho da div de cada grupo** — e `first:` casa com `:first-child`, então ele se escondia em todos os grupos, sempre. Os quatro blocos apareciam como uma lista uniforme, enquanto o código e o `nav-items.ts` afirmavam que o agrupamento era "a única pista de arquitetura que o rail dá". A pista não existia. Agora a margem vive na div do grupo (`mt-3 first:mt-0`), onde `first:` de fato aponta para o primeiro grupo. **Lição que vale além daqui: `first:`/`last:` num elemento que é sempre o primeiro filho do próprio contêiner é sempre-verdadeiro ou sempre-falso — a variante precisa morar no elemento que se repete, não no que o envolve.**

- **Os blocos do rail têm NOME, não só espaço** (19/ago, junto com o rótulo). `ROTULOS_DE_GRUPO` em `nav-items.ts`: **Panorama · Quem é o cliente · O que ele usa · Quanto isso vale · Ferramentas**. Os três do meio são a pergunta que o negócio faz, que já era a razão do agrupamento e vivia só em comentário — com o rail rotulado, 12px de ar entre blocos lê como acidente, e o nome põe a ordem na tela. É `Record<GrupoDeNav, string>` de propósito: grupo novo sem nome é erro de compilação, não bloco anônimo. O `RotuloDeGrupo` é `h2` + `ul`/`li` — quem usa leitor de tela navega por cabeçalho e ouve quantos itens o bloco tem.
  - **A gaveta mobile repete os mesmos blocos.** Ela É o rail abaixo de `lg`; agrupar numa largura e virar lista corrida na outra obriga a reaprender a navegação ao girar o aparelho.
  - Caixa alta com `tracking-wide` **obrigatório**: o base aplica `letter-spacing: -0.011em`, e caixa alta a 11px com tracking negativo fecha as letras. A regra "sem uppercase" do DS é da **pílula/chip**, que é controle — aqui o elemento é cabeçalho de seção e precisa não competir com os itens.
- ⚠️ **Elemento `sticky` mais alto que a área visível tem o pé INALCANÇÁVEL** — ele nunca sai do lugar, então rolar a página não revela o resto. Os nomes de grupo levaram o rail a **758px de conteúdo contra os 664px** de uma tela de 768px de altura (1366×768 é laptop comum), e "Design system" ficaria fora para sempre. A saída é `max-h` **igual** ao `min-h` mais `overflow-y-auto` no `AppRail` (em `app-layout.tsx`): o rail tem exatamente a altura da área visível e rola por dentro quando não cabe; em tela alta nada muda, porque não sobra o que rolar. **Módulo novo passa a caber por construção** — antes, o 14º item teria sumido em silêncio.
- `AppRail` (`app-rail.tsx`) — rail vertical branco, ícone **+ rótulo**, agrupado e **nomeado** por `GRUPOS_DE_NAV`, ferramentas no rodapé. Item ativo é pílula preenchida (`bg-primary`), não fundo mais claro: sobre o cromo branco a diferença de alfa que funcionava na barra navy é quase invisível. **O mesmo vale para a gaveta** — ela usava `bg-accent`, que sobre o fundo dela dava **1,03:1** (1,15:1 no escuro), e o único canal real de estado era a cor do texto.
- `AppBarra` (`app-barra.tsx`) — uma barra branca só: marca · aba ativa · abas inativas · busca · frescor · tema · conta. Substitui `app-header.tsx` (navy) e absorve `modulo-tabs.tsx`. **`sticky` abaixo de `lg`**, pelo mesmo motivo que o rail é sticky no desktop: no celular a navegação inteira mora no botão de gaveta dela, então barra que sobe leva a navegação junto.
...
- `SecaoDeAnalise` (`layout/`) — **contêiner cinza que agrupa cards que respondem à mesma pergunta**, com cabeçalho próprio (título + controle da seção + ações). Título com `line-clamp-2`, igual ao `CardCabecalho` — a decisão já estava escrita no irmão ("título cortado é título que mente") e invertida aqui, com 33 dos 43 títulos terminando em reticências a 375px. É a peça que faltava: sem ela a página é uma pilha de cards sem hierarquia, e é ela que o catálogo de análises vai preencher.
...
Novas: `aba-canal.tsx` · `app-barra.tsx` · `app-rail.tsx` · `aba-do-modulo.ts` (estado da aba, lido da URL) · `cabecalho-de-modulo.tsx` · `secao-de-analise.tsx` · `fronteira-de-erro.tsx` · `ui-marca/controle-segmentado.tsx` · `ui-marca/frescor-do-dado.tsx` · `tabela/lista-de-acao.tsx` · `lib/periodo.ts`.

**`FronteiraDeErro` (`layout/fronteira-de-erro.tsx`) — DUAS, aninhadas.** A de dentro vive no `AppLayout` e envolve só o `Outlet`: erro de módulo não pode custar a barra e o rail, senão a saída de uma tela quebrada vira recarregar a página para conseguir ir a outro lugar. A de fora envolve as rotas e cobre login, 404 e o próprio shell. Classe, porque React 19 segue sem hook para erro de renderização.

- ⚠️ **`key={pathname}` no ponto de uso é a parte que falha CALADA.** Sem ela o estado de erro sobrevive à navegação seguinte e a próxima tela nasce quebrada sem nunca ter sido renderizada — o defeito passa a parecer estar em toda parte. Fronteira sem chave é pior que nenhuma, porque parece estar funcionando. Travado em `contrato-de-shell.test.ts`.
- É `pathname` e não a location inteira: trocar `?aba=` ou `?periodo=` não remonta nada.
...
- **`AbaDeDados`: nenhuma consulta nova, e nasce FECHADA.** Recebe as linhas JÁ carregadas pelos mesmos hooks que desenham os gráficos, e vive no fim da aba `Gráficos`.
  - **Um `Collapsible` só, fechado**, com "N funções · M linhas" fora do dobrável. São 5 a 12 `TabelaCard` completos por tela — **88 blocos somando as dez**, contra 80 de cards de conteúdo: em Clientes e CS a camada de auditoria pesava mais que a análise inteira. Auditoria é para quando se duvida, não para o caminho de leitura. A contagem fica visível pela mesma régua do `AcordeaoDeAchados`: atrás do clique vai a profundidade, nunca o número que dá sentido ao título.
  - ⚠️ **Os cards dela NÃO declaram `nivel`** — não são degraus da escada, são a prova dos degraus. Enquanto declaravam `descritivo` fixo, as páginas somavam 19 descritivos e o DOM entregava **107** contra um teto de 3 por tela, porque `escada.test.ts` varre `features/**/*-page.tsx` e o componente mora em `components/`. A régua não estava errada: estava medindo o arquivo errado. Se ela relesse o banco, passaria a existir uma segunda consulta capaz de divergir do card acima — o defeito que o projeto vem fechando em outras camadas. A garantia é estrutural: é o mesmo objeto em memória.
...
- **Régua de densidade** (`src/lib/densidade.ts`), a irmã da escada com a pergunta invertida: a escada responde "esta tela é rasa?", esta responde **"esta tela é lida?"**. Máximo 9 cards de conteúdo · mínimo 2 cards por `SecaoDeAnalise` · máximo 5 seções · mínimo 1 descritivo · máximo **240 caracteres** de prosa de seção. Mais duas regras de ORDEM: a primeira seção não pode ser só descritiva, e a última não pode ser a única com o prescritivo. Adoção por lista (`TELAS_NA_DENSIDADE`), como a escada — **7 das 10 telas em 19/ago**; faltam clientes, CS e receita.
  - **Cada número saiu de medição.** 9 é o primeiro inteiro acima da maior tela que nenhum auditor reprovou; 2 é o menor grupo que existe (12 das 43 seções tinham um card só); 5 sai da distribuição medida e as duas telas com 6 são exatamente as que os auditores mandaram fundir; 240 é 3 linhas a 80ch, uma a mais que a régua de card.
  - ⚠️ **A ordem da tela caiu da ordem de um array, e ninguém decidiu isso.** `escada.ts` declara `NIVEIS` em ordem ascendente e a `REGUA` se lê de cima para baixo, então quem monta a tela para satisfazer aquela lista monta na ordem da lista — e seis das oito telas com nível terminavam no único card prescritivo. Uma regra de COMPOSIÇÃO virou, em silêncio, uma regra de SEQUÊNCIA. **O que a régua garante não é uma ordem única: é que a ordem tenha sido escolhida.**
  - **Exceção se declara no fonte da PÁGINA** (`DENSIDADE_DECLARADA: <regras> — <motivo>`), no padrão do `comment on function`, e **sem motivo escrito não vale**. Hoje há duas, as duas para `cardsPorSecaoNoMinimo`: o meta-card de rastreio na Visão Geral e o de sessões infladas na Jornada. O critério para fundir é a PERGUNTA, nunca o eixo — em Jornada a fusão tentadora era com a seção que usa o mesmo eixo, e teria agrupado por eixo em vez de por pergunta.
  - **`TETO_POR_TELA` é a catraca ao contrário**, e vale para as dez. Em 130 commits a contagem de cards caiu UMA vez; sem trava, a faxina se desfaz em seis meses sem ninguém decidir. Baixar um teto é de graça, subir exige editar o arquivo e escrever por quê. Conta cards **e colunas** (`<TableHead>`) porque os dois eixos apontam telas OPOSTAS: por cards CS e clientes são as piores e soluções está limpa; por números impressos **soluções é a pior do produto**, com 30 colunas. Tela de poucos cards e tabelas largas passa em toda régua que conta blocos — e é a que o leitor chama de poluída.
  - ⚠️ **Encurtar prosa APAGA RÉGUA.** Aconteceu três vezes: quando a régua foi para o `nav-items.ts`, e duas vezes em 19/ago — o corte de `bi_orgs_risco` (25 de 1.031 orgs) e o de `bi_fluxo_da_tela` (`limit 10` chumbado no SQL) existiam APENAS na prosa da seção. Antes de cortar uma cláusula, achar o destino dela: `description` do card, `limiteDaFonte={N}` na TabelaLonga, ou a `regua` do `nav-items`. Sem destino, corte outra coisa. **`limiteDaFonte` é melhor que a prosa era**: só anuncia quando o corte de fato morde.
...
- ⚠️ **A régua `e_cliente` é da RPC, não do mart.** Os fatos guardam todo mundo — admin, interno e teste — e quem filtra é quem lê. Auditado em 13/08: 5 de 59 RPCs de grão-cliente liam sem a régua, e o desvio ia de 2,3% (aula) a **30,8%** (consultor). Na tela de IA isso não era arredondamento: a ordem das faixas de profundidade mudou, "parou na 1ª mensagem" caiu de 3º para 5º maior. **RPC nova que toca fato de grão-cliente junta `dim_usuario` com `e_cliente` — sem exceção não declarada.**
  ...
  - ⚠️ **E existe o falso NEGATIVO, que é o caro: a régua é por CTE, não por função.** `bi_ia_adocao` citava `e_cliente` em dois dos três CTEs e o terceiro — a interseção — contava todo mundo. Passa em qualquer grep, porque o nome da régua está lá duas vezes. Eram 45 não-clientes numa linha que a tela desenhava ao lado de duas filtradas, e o defeito só apareceu recomputando a função bloco a bloco. **Auditoria de `e_cliente` se faz lendo cada leitura de fato, não procurando o nome no corpo.**
